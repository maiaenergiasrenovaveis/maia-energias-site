---
title: "Como funciona o diagnóstico energético gratuito da Maia Energias, passo a passo"
description: "Entenda as etapas do diagnóstico energético que identifica desperdícios e oportunidades de redução na conta de energia de empresas com contas acima de R$50 mil por mês."
publishDate: 2026-08-08
audience: empresas
---

O **diagnóstico energético** é o primeiro passo para saber se sua empresa está pagando mais do que deveria pela energia — ele analisa as últimas 12 contas e a estrutura elétrica da instalação para identificar desperdícios reais, sem custo para solicitar a análise preliminar.

## Etapa 1 — Envio das últimas 12 contas de energia

Basta enviar as últimas 12 contas de energia da empresa. Com esse histórico, a Maia consegue fazer uma primeira leitura do perfil de consumo — sazonalidade, horários de pico, e indícios de distorções na demanda contratada ou no fator de potência.

## Etapa 2 — Análise técnica do consumo e da instalação

A partir das contas e, quando necessário, de uma visita técnica, a Maia analisa:

- **Demanda contratada** — se está dimensionada corretamente para o consumo real;
- **Fator de potência** — se está dentro do mínimo exigido pela distribuidora;
- **Energia reativa excedente** — se há cobrança adicional por ultrapassagem;
- **Consumo no horário de ponta** — se há oportunidade de gestão de carga;
- **Condições da instalação elétrica** — estado de subestações, equipamentos e possibilidade de geração própria ou armazenamento.

## Etapa 3 — Apresentação das oportunidades identificadas

Com o diagnóstico concluído, a Maia apresenta as oportunidades identificadas com os investimentos e as economias estimadas para cada medida — sempre calculadas individualmente a partir do perfil real da empresa, nunca como percentual fixo ou garantido.

## Etapa 4 — Acompanhamento da implementação

Aprovadas as medidas, a Maia acompanha a implementação das soluções — que podem incluir revisão da demanda contratada, correção do fator de potência, geração solar, sistemas de armazenamento de energia (BESS) ou modernização de subestações, conforme o que o diagnóstico apontar como prioritário para o perfil da empresa.

## Quanto tempo leva

O prazo varia conforme a complexidade da instalação e o volume de dados analisados, mas a primeira leitura a partir das 12 contas de energia costuma ser rápida — o diagnóstico técnico mais aprofundado (com visita, se necessária) é que define o cronograma completo.

Quer começar o diagnóstico da sua empresa? [Envie as últimas 12 contas de energia](/contato) para uma análise preliminar.
